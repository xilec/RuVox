...
// ── Import commands (text-import spec, #224) ──────────────────────────────────
...
/// dialog, #223). Returns None when the user cancels.
#[tauri::command]
pub async fn pick_import_file() -> CmdResult<Option<String>> {
    tokio::task::spawn_blocking(|| {
        rfd::FileDialog::new()
            .add_filter("Текстовые файлы", &["txt", "md", "html", "htm"])
            .pick_file()
...
async fn download_page(client: &reqwest::Client, url: &str) -> CmdResult<FetchUrlTextResult> {
    ...
    let decoded = crate::import::detect_and_decode(&bytes).map_err(map_import_error)?;
