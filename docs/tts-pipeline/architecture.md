...
│  - Mermaid-блоки → "Тут мермэйд диаграмма"                  │
...
|---|-----|---------|----------------|
| 0 | MERMAID | ` ```mermaid...``` ` | → "Тут мермэйд диаграмма" |
...
Markdown-ссылки: URL убирается, остаётся только текст ссылки.
Английские слова в тексте ссылки транслитерируются как обычно.

```markdown
[Fun with Dada](https://example.com/path)
```
→ "фан виз дада"

> **Важно:** "голые" URL (без markdown-обёртки) продолжают озвучиваться:
> `https://example.com` → "эйч ти ти пи эс двоеточие слэш слэш example точка ком"

### Mermaid-диаграммы

Mermaid-блоки не содержат читаемого текста — заменяются маркером:
...
Это позволяет пользователю приостановить чтение и рассмотреть диаграмму в UI.
Замена одинакова для обоих режимов (full/brief).
