# Viewer Copy Actions Specification

## Purpose

Covers the copy affordances of the `TextViewer`: the custom right-click
context menu (the webview provides no native one), copying link addresses,
selected text, and images (bitmap + address) to the clipboard, and the
Ctrl+C / Cmd+C hotkey for links. Available in every display mode (plain,
markdown, HTML); all clipboard writes go through
