...
**Функционал:**
- Отображение Markdown (рендеринг в HTML)
- Read-only режим
...
class TextViewerWidget(QTextBrowser):
    ...
    def set_entry(self, entry: TextEntry):
        ...
        html = markdown.markdown(entry.original_text)
        self.setHtml(html)
...
**Подсветка:**
- Текущее слово: жёлтый фон + подчёркивание
- ±2 слова от текущего: светло-жёлтый фон (опционально, если timestamps позволяют)
