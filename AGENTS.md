...
- **Primary path is the CLI:** `nix develop -c pnpm dlx @fission-ai/openspec <cmd>` (`list`, `show`, `validate`, `new`, `archive`, …). Slash commands (`/opsx:*` in Claude Code, `/skill:openspec-*` in Kimi Code, `/openspec-*` in ZCode) are convenience wrappers around the same CLI.
...
- The `openspec-*` skills are generated by the CLI (into `.claude/skills/`) but carry repo-specific notes (the `nix develop -c pnpm dlx` invocation path, links to `ai/rules/`) — re-apply those notes after regenerating skills via an openspec CLI update. Tool-adapted copies live in `.agents/skills/` (Kimi Code; `/skill:` invocations, `TodoList`) and `.zcode/skills/` (ZCode; `/`-invocations, `when_to_use` frontmatter) — the ZCode copies shadow the `.agents` ones in ZCode sessions, so update both alongside the Claude originals.
