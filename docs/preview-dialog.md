...
## Связь с edit mode (FF 1.2)

Preview-диалог даёт редактировать **до** синтеза (изменения уходят в `addTextEntry` как готовый текст; в storage записывается как `original_text`).

Edit mode в `TextViewer` (FF 1.2) даёт редактировать **после** синтеза (изменения сохраняются в `edited_text`). Однако фича пересинтеза существующего entry в текущем UI **не реализована** — `update_entry_edited_text` пишет правки в storage, но re-synth не запускает; следующее воспроизведение проигрывает прежний WAV. Подробнее в [use-cases.md § 8](use-cases.md#8-edit-mode-ff-12--частично-реализовано).
