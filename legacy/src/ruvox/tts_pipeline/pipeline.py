...
from ruvox.tts_pipeline.tracked_text import CharMapping, TrackedText
from ruvox.tts_pipeline.word_mapping import WordMapping, build_word_mapping
...
class TTSPipeline:
    ...
    _RE_SIZE = re.compile(
        r"\b(\d+(?:\.\d+)?)\s*(KB|MB|GB|TB|ms|sec|min|hr|px|em|rem|vh|vw|кб|мб|гб|тб)\b",
        re.IGNORECASE,
    )
    ...
    def _process_code_identifiers_tracked(self, tracked: TrackedText) -> None:
        ...
        # CamelCase
        tracked.sub(r"\b([a-z]+(?:[A-Z][a-z]*)+)\b", lambda m: self.code_normalizer.normalize_camel_case(m.group(0)))

        # PascalCase
        tracked.sub(
            r"\b([A-Z][a-z]+(?:[A-Z][a-z]+)+)\b", lambda m: self.code_normalizer.normalize_camel_case(m.group(0))
        )
