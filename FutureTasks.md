...
Настроить GitHub Actions для автоматической проверки при push/PR:

1. **Линтинг и форматирование**
   - Проверка стиля кода (ruff или flake8)
   - Проверка форматирования (ruff format или black)
   - Проверка типов (mypy) — опционально

2. **Прогон тестов без дисплея**
   - Запускать тесты, не требующие графического окружения (TTS pipeline, error handling)
   - Исключить UI-тесты, зависящие от QApplication/WebEngine (`tests/ui/`)
   - Добавить pytest-маркер `@pytest.mark.no_display` или использовать `--ignore=tests/ui`
   - Настроить coverage-отчёт

3. **Бейджи в README**
   - Статус CI (passing/failing)
