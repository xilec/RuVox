...
- **Формат источника** в диалоге предпросмотра: «Авто», «Обычный текст», «Markdown» или «HTML». По умолчанию включён «Авто» — RuVox сам распознаёт формат: из HTML извлекается читаемый текст, а разметка Markdown (заголовки, списки, блоки кода) обрабатывается по смыслу, а не зачитывается посимвольно. Если автоопределение ошиблось, формат можно выбрать вручную.
- **Директива `<!-- ruvox-code: brief -->`** в тексте переключает озвучку блоков кода на краткое описание («далее следует пример кода на Python»); директива `<!-- ruvox-code: full -->` возвращает полное чтение. По умолчанию код читается полностью.
