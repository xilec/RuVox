...
export const ru = {
  ...
  'preview.explain.details':
    'Переписывается всё, что синтезатор не читает напрямую: идентификаторы (getUserData → «гет юзер дата»), сокращения (API → «эй пи ай»), числа и даты, URL и email, операторы и спецсимволы, содержимое блоков кода.\nФормат источника (Авто / Обычный текст / Markdown / HTML) определяет подготовку текста: из HTML извлекается читаемый текст, в Markdown проговариваются блоки кода, а mermaid-диаграммы заменяются фразой «Тут мермэйд диаграмма». Директива <!-- ruvox-code: brief --> включает краткое описание блоков кода вместо полного чтения.',
