...
> **Все команды запускаются через `nix-shell --run "..."`** — `uv`, `ruff` и прочие инструменты доступны только внутри nix-shell.

```bash
nix-shell --run "uv sync --extra ui --extra tts"   # Зависимости
nix-shell --run "uv run ruvox"                      # Запуск UI
nix-shell --run "uv run pytest"                     # Тесты
nix-shell --run "./scripts/test.sh"                 # Тесты через скрипт
```
...
```bash
# Тесты
nix-shell --run "./scripts/test.sh"                   # Все тесты
nix-shell --run "./scripts/test.sh module numbers"     # Конкретный модуль
nix-shell --run "./scripts/test.sh match 'url'"        # По паттерну

# Приложение
nix-shell --run "uv run ruvox"                         # UI
nix-shell --run "uv run python scripts/tts_generate.py FILE"  # Генерация аудио
```
...
- Код должен соответствовать стилю, проверяемому `ruff`. Перед коммитом запускай `nix-shell --run "ruff check ."` и `nix-shell --run "ruff format --check ."`, исправляй ошибки.
- Опциональная проверка типов: `nix-shell --run "mypy src/ruvox/tts_pipeline/"`. Проверяет только аннотированный код, неаннотированные функции пропускает.
...
```bash
nix-shell --run "uv run pytest --collect-only -q"
```
