...
`build_sequence` (`frontend/text.rs:149-172`) is the last point where the correspondence `sequence[i] ↔ sentence.char_at(i-1)` (offset by sos) is still direct. It is extended to also emit, alongside the ids, the `char` each id was produced from (sos/eos included; dropped symbols simply don't appear). Timestamps are then computed by walking the original chunk text's words letter-by-letter against this symbol stream (skipping non-letter symbols such as `+`, `^`, punctuation, spaces, sos/eos; `ё` ≡ `е` because the accentor may substitute them). A word's range runs from its first matched letter's frame start to its last matched letter's frame end; punctuation pauses between words become gaps. Words that match nothing get `start == end` at the current cursor.

The stream letters are an in-order subsequence of the original text's letters — the frontend only *drops* characters it cannot map (latin, digits, symbols), it never reorders and never drops Cyrillic. So a word letter that mismatches the current stream symbol was dropped before synthesis: it is skipped without consuming the stream symbol, and the cursor only advances on matches. This keeps one unmapped word from misaligning the whole chunk: the production text contained "get_variablesслэш" (pipeline left the latin prefix un-transliterated, frontend dropped it), and breaking the word loop on the first mismatch shifted every following word by one — a ~70-word highlight jump (second production failure found in the manual pass, caught by the playback-emulation assert before the fix).
...
*`+` stress markers carry audio.* The bundle-gated suite showed the accentor emits `+` immediately before the stressed vowel (including single-vowel words via `stress_single_vowel`) and the model renders real frames for it (e.g. `+я`: sos 5 frames, `+` 11 frames, `я` 9 frames). A `+` run **directly attached** to the word's first letter therefore opens the word's range; markers elsewhere are covered by the range via the cumulative timeline. A *standalone* literal `+` (user text like "правило + команда", spaces around it) is NOT a stress marker: folding it into the next word cascaded every following word into zero-length timestamps (production failure found in the manual pass). Attachment is decided by contiguity: a `+` run directly followed by a letter.
