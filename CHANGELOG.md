...
### Added
- **Движок «Silero (нативный)»** — третий TTS-движок: Silero v5 работает в процессе приложения на ONNX Runtime (крейт `silero-native/`), без Python-sidecar'а. Модельный бандл (~230 МБ) скачивается по требованию из GitHub Releases с проверкой sha256; полный паритет с Python-версией (автоударения, омографы, ё). Python-версия Silero сохраняется как запасной вариант.
- **Поддержка Windows 10 22H2+ / 11 (x86_64)** — NSIS-установщик со встроенным бутстраппером WebView2; бандленый mpv и ONNX Runtime подкладываются сборкой (pinned-ресурсы, проверка sha256).
- **Автообновления** — `tauri-plugin-updater`: проверка из настроек и в фоне, подписанные апдейты (приватный ключ в GitHub Secrets), дельта по `latest.json` из GitHub Releases.
- **Release pipeline** — тег `v*` запускает GitHub Actions job: сборка под `windows-latest`, подпись апдейтов, публикация draft-релиза с установщиком и `latest.json`.

### Changed
- **Движок по умолчанию — «Silero (нативный)»** (голос aidar, 24000 Гц) вместо Piper. Существующие конфигурации не затронуты: новые значения применяются только к отсутствующим ключам `config.json`; если бандл модели не скачан, приложение как и раньше запускается на Piper, а бандл предлагается скачать в настройках.
